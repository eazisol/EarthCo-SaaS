...
const RisingCaneTable = () => {
  ...
  const fetchWeeklyReports = async () => {
    try {
      const res = await axios.get(
        `${baseUrl}/api/WeeklyReport/GetWeeklyReportRCList?CustomerId=${customerParam}`,
        { headers }
      );
  ...
  // Permissions for Monthly Report - Raising Canes
                      ...
                      filteredReports
                          ...
                          >
                            ...
                            <TableCell
                              onClick={() => {
                                if (!canEdit) {
                                  navigate(
                                    `/Weekly-Reports/rising-canes-preview?id=${report.WeeklyReportRCId}${report.CustomerId ? `&CustomerId=${report.CustomerId}` : ''}`
                                  );
                                  return;
                                }
                                if (loggedInUser.userRole=="2") {
                                  navigate(
                                    `/Weekly-Reports/rising-canes-preview?id=${report.WeeklyReportRCId}&CustomerId=${report.CustomerId}`
                                  );
                                  return
                                }
                                navigate(
                        ...
                        <TableCell align="right">
                          {canDelete ? (
                            ...
                            <Tooltip title="You don't have permission to delete monthly reports" arrow>
                              <span>
                                <DeleteModal
                                  endPoint={`WeeklyReport/DeleteWeeklyReportRC`}
                                  successFun={fetchWeeklyReports}
